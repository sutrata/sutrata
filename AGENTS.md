...
The editor takes its dependencies on the outside world through injected interfaces rather than importing them directly, so an embedder (the OSS app, eventually Sutrata Cloud) can swap them without forking the editor (OSS spec §11.5). All follow one pattern: interface + React context + `useX()` hook in `extensions/`, a prop on `DocumentProvider`, concrete implementation in the embedder.

- **`StorageAdapter`** (`extensions/storage-adapter.ts`) — required prop. `DocumentContext.tsx` and `StyleDialog.tsx` go through `useStorageAdapter()`. `@sutrata/app` supplies `localStorageAdapter`.
- **`AIProvider`** (`extensions/ai-provider.ts`) — optional `aiProvider` prop. Components call `useAI()`, which returns `null` when there is no provider, the session can't edit, or `featureFlags.ai === false`; AI UI is hidden then. The editor owns the *prompts* (`ai/prompts.ts`, they describe Sutra) and output cleanup (`ai/clean-response.ts`); the provider only transports. `@sutrata/app` supplies the bring-your-own-key provider (`app/src/ai/`: `ai-client.ts`, API-key storage, provider list generator, onboarding dialog, AI settings section). The editor must not import any provider/key code.
...
- **Registries** (`extensions/{panel,command,export}-registry.ts`, generic store in `registry-store.ts`, `createRegistries()` in `registries.ts`) — optional `panels` / `commands` / `exporters` props (a live registry or a fixed list); `register()` returns an unregister function. The editor's **built-ins are registered the same way** and merged by `withBuiltins()` (a registered entry with a built-in's id replaces it): element/mark/history commands in `shell/builtin-commands.tsx` (the element toolbar renders `menu: 'toolbar'` commands; Alt+Shift element shortcuts are dispatched from the registry in `DocumentContext`), and every export format/report in `file/builtin-exporters.ts` (`ExportDialog` renders from that single list). Panel locations: `sidebar` (tabs beside the built-in navigator; the layout is unchanged when none are registered), `inspector` (right-hand panel), `appbar`, `settings`; they render with `{ ast, sceneIds, activeSceneId }` from `shell/use-panel-context.ts`. Adding a new built-in toolbar action or export format means adding it to those builtin lists, not to the components.
...
2. **Editor serializer** (`packages/editor/src/editor/prosemirror-to-sutra.ts`) — regenerates Sutra from the ProseMirror doc (the editor doc does not carry per-block `raw`; only `title_page` keeps its frontmatter source in a `raw` attr and re-emits unchanged keys verbatim). This path is used after the user *edits*. It is **AST-lossless, not byte-exact**: `parse → sutraToProsemirror → prosemirrorToSutra → parse` must yield an AST equal to the first parse (ignoring `raw`). `packages/editor/tests/lossless-round-trip.test.ts` enforces this over the conformance corpus and a `fast-check` property test — anything the AST carries (attribute blocks, list metadata, registry metadata, nested emphasis) needs a node or attr in `schema.ts` and both bridge directions.
